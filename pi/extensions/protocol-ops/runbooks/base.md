Protocol Ops provides knowledge and a human-confirmed scope for audited reads; it never authorizes arbitrary shell or mutation by itself. A typed mutation may run only through its action-specific tool, deterministic validation, and that tool's machine-configured risk policy.
...
- Keep discover → plan → apply → verify separate. Only an action-specific tool may cross into apply; its typed policy decision—not a runbook, checkpoint, reviewer, task scope, or model-supplied role—is the mutation authority.
